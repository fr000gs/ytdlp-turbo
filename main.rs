...
fn ask_if_format() -> bool {
    to_boolean(&read_input("Display formats? [Y/n]: ")
    .parse()
    .unwrap_or("y".to_string())
    .to_lowercase())
}
...
fn download_playlist(url: &str) {
    ...
    fn download_video(url: &str) {
        ...
        fn select_format() -> String {
            ...
            let format_input = read_input("Enter format video+audio or press Enter for default format").parse().unwrap_or(default_format.clone());
            ...
            let confirmation = read_input(&("Selected format:".to_owned()
            + &format_code.clone()
            + ". Confirm? (Y/n): "))
            .parse().
            unwrap_or("y".to_string())
